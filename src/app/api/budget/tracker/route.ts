...
export async function GET(request: Request) {
  try {
    ...
    let totalBudget = 0;
    let totalSpent = 0;
    const projectBudgets: Array<{id: string; title: string; budget: number; spent: number; remaining: number; percentage: number; status: string}> = [];
    const alerts: Array<{projectId: string; projectTitle: string; type: string; message: string; severity: string}> = [];
