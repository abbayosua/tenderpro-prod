...
export default function TenderProApp() {
  ...
  const [selectedDispute, setSelectedDispute] = useState<{ id: string } | null>(null);
  ...
  useEffect(() => {
    if (user && activeTab === 'dashboard') {
      const timer = setTimeout(() => {
        const completed = localStorage.getItem('tenderpro_onboarding_complete');
        if (!completed) {
  ...
  if (activeTab === 'dashboard' && user?.role === 'OWNER') {
    return (
      <>
        {showOnboarding && user && activeTab === 'dashboard' && (
          <OnboardingWizard
            onComplete={() => {
              localStorage.setItem('tenderpro_onboarding_complete', 'true');
        ...
        <OwnerDashboard
          ...
          onShowDispute={(disputeId) => {
            setSelectedDispute(disputeId ? { id: disputeId } : null);
            setDisputeOpen(true);
          }}
        />
        <CreateProjectModal
